...
function fetchSchema(tabId) {
  return new Promise((resolve, reject) => {
    if (latestSchema && latestSchema.length > 0) {
      console.log("🗂️ Using cached schema");
      return resolve(latestSchema);
    }

    console.log("🕵️ Requesting schema from content script...");

    function onSchemaMsg(message, sender) {
      if (message.type === "SCHEMA_UPDATE" && sender.tab.id === tabId) {
        console.log("📥 Received schema from content script:", message.schema);
    ...
    chrome.tabs.sendMessage(tabId, { type: "GET_SCHEMA" }, (response) => {
      ...
      console.log("📤 Sent GET_SCHEMA to content script");
    });
...
// Main form‑fill handler
async function handleFormFill(settings, tabId, sendResponse) {
  try {
    console.log("🔄 [background] Starting form fill process");
    latestSchema = null;

    console.log("📋 [background] Fetching form schema...");
    ...
    console.log(`📋 [background] Got schema with ${schema.length} fields`);
    ...
    console.log("🎯 [background] Parsed answers:", answers);
    ...
    console.log("✅ [background] Form fill completed successfully");
    ...
    console.error("❌ [background] Form fill failed:", err);

    chrome.tabs.sendMessage(tabId, {
      type: "FILL_ERROR",
      error: err.message
    }).catch(contentError => {
      console.warn("⚠️ [background] Failed to send error to content script:", contentError);
    });
...
chrome.runtime.onMessage.addListener((message, sender, sendResponse) => {
  if (message.type === "SCHEMA_UPDATE") {
    latestSchema = message.schema;
    console.log("🔄 [background] Schema updated:", latestSchema?.length || 0, "fields");
    sendResponse({ success: true }); // ✅ Fix applied here
    return true; // ✅ Keep channel open until sendResponse is called
  }

  if (message.type === "FILL_FORM") {
    console.log("🔔 [background] Got FILL_FORM from popup");
...
chrome.action.onClicked.addListener(async (tab) => {
  if (tab.url.includes("docs.google.com/forms")) {
    ...
    if (settings.geminiKey && settings.groqKey && settings.pineconeKey && settings.pineconeEnv) {
      ...
      console.warn("⚠️ API keys missing; open popup to configure them");
...
chrome.runtime.onInstalled.addListener(async () => {
  ...
  console.log(`🔍 [background] Found ${tabs.length} Google Forms tabs to inject`);
  ...
  for (let tab of tabs) {
    try {
      ...
      console.log(`✅ [background] Injected content.js into tab ${tab.id}`);
    } catch (e) {
      console.warn(`⚠️ [background] Failed to inject into tab ${tab.id}:`, e);
    }
  }
});

chrome.runtime.onInstalled.addListener(() => console.log("🚀 [background] Extension installed"));
chrome.runtime.onStartup.addListener(() => console.log("🔄 [background] Extension started"));

chrome.runtime.onStartup.addListener(async () => {
  try {
    const response = await fetch(`${API_BASE_URL}/`);
    if (response.ok) {
      console.log("✅ [background] Backend connectivity verified");
    } else {
      console.warn("⚠️ [background] Backend responded with error:", response.status);
    }
  } catch (error) {
    console.warn("⚠️ [background] Backend connectivity test failed:", error.message);
  }
});

...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  const settings = await chrome.storage.sync.get([
    'geminiKey', 'groqKey', 'pineconeKey', 'pineconeEnv'
  ]);
...
document.getElementById('uploadPdfs').addEventListener('click', async () => {
  ...
  const settings = await chrome.storage.sync.get(['geminiKey', 'pineconeKey', 'pineconeEnv']);
  ...
  if (!settings.geminiKey || !settings.pineconeKey || !settings.pineconeEnv) {
    showStatus('Please save all required API keys and environment first', 'error');
    return;
  }
...
document.getElementById('saveSettings').addEventListener('click', async () => {
  ...
  if (!settings.geminiKey || !settings.groqKey || !settings.pineconeKey || !settings.pineconeEnv) {
...
document.getElementById('fillForm').addEventListener('click', async () => {
  ...
  try {
    ...
    if (!settings.geminiKey || !settings.groqKey || !settings.pineconeKey || !settings.pineconeEnv) {
      throw new Error('Please save all API keys and Pinecone environment first');
    }
